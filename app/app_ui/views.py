...
WORKFLOW_DISPLAY_LABELS = {
    "Lite workflow": t('Lite workflow (no refine)'),
    "Full workflow": t('Full workflow (all stages)'),
...
def sync_workflow_dependent_state(session_id: str, preset: str) -> None:
    ...
    st.session_state[f"{session_id}_source_mode"] = t('Upload new files')
    st.session_state[tracker_key] = preset
...
def workflow_preview_badge(active: bool | None) -> tuple[str, str]:
    if active is True:
        return "ON", t('This stage will run.')
    if active is False:
        return "OFF", t('This stage will be skipped.')
    return "BASE", t('This is a shared workflow stage.')
...
SCAN_TYPE_SPECS = {
    "bond": {
        ...
        "help": t('Bond scan requires 2 atom indices. Example: 0,1'),
    },
    "angle": {
        "example": "0,1,2",
        "count": 3,
        ...
        "help": t('Angle scan requires 3 atom indices. Example: 0,1,2'),
    },
    "dihedral": {
        "example": "0,1,2,3",
        "count": 4,
        ...
        "help": t('Dihedral scan requires 4 atom indices. Example: 0,1,2,3'),
    },
}
...
SCAN_PRESET_OPTIONS = {
    "custom": {
        ...
        "step_size": None,
        "steps": None,
        "description": t('Manually set the scan type, atom indices, range, and step spacing.'),
    },
    "dihedral_twist_pm360": {
        ...
        "step_size": 10.0,
        "steps": None,
        "description": t('Select four atoms and scan -360 -> +360 deg around the current dihedral.'),
    },
    "dihedral_twist_plus360": {
        ...
        "step_size": 10.0,
        "steps": None,
        "description": t('Rotate from the current dihedral to +360 deg for conformer searches or rotational barriers.'),
    },
    "dihedral_twist_minus360": {
        ...
        "step_size": 10.0,
        "steps": None,
        "description": t('Rotate in the reverse direction from the current dihedral to -360 deg.'),
    },
    "angle_wag_60_180": {
        ...
        "step_size": 5.0,
        "steps": None,
        "description": t('Scan a three-atom angle over the absolute range 60 -> 180 deg.'),
    },
    "angle_wag_pm30": {
        ...
        "step_size": 5.0,
        "steps": None,
        "description": t('Wag around the central atom over +/-30 deg around the current angle.'),
    },
    "bond_stretch_plus1": {
        ...
        "step_size": 0.05,
        "steps": None,
        "description": t('Stretch a two-atom distance from the current value to +1.0 Å.'),
    },
    "bond_compress_minus05": {
        ...
        "step_size": 0.05,
        "steps": None,
        "description": t('Compress a two-atom distance from the current value to -0.5 Å.'),
    },
}
...
SCAN_STEP_MODE_OPTIONS = {
    "steps": t('Specify by number of steps'),
    "step_size": t('Specify by step size'),
}
...
def current_scan_coordinate_from_source(
    *,
    ...
    try:
        if source_mode == t('Use built-in sample') and sample_case:
            reactant_path, _ = sample_case_files(sample_case)
            if reactant_path is None:
                return None, t('Sample reactant XYZ was not found.')
            text = reactant_path.read_text(encoding="utf-8")
        elif source_mode == t('Upload new files') and reactant_file is not None:
...
def resolve_scan_settings(module_settings: dict[str, object], current_value: float | None) -> tuple[dict[str, object], list[str], list[str]]:
    ...
    if range_mode == "auto_to_absolute":
        ...
        if current_value is None:
            errors.append(t('Relative SCAN requires reading the current value from the reactant XYZ. Check the reactant file and atom indices.'))
            current_value = 0.0
        start_auto = False
    ...
    if step_mode == "step_size":
        ...
        if step_size <= 0:
            errors.append(t('SCAN step size must be greater than 0.'))
        else:
            if start_auto and current_value is None:
                errors.append(t('Step-size mode requires reading the current value from the reactant XYZ.'))
            else:
                effective_start = current_value if start_auto else float(start_val)
                span = abs(float(end_val) - float(effective_start))
                if span <= 0:
                    errors.append(t('SCAN range is zero. Change the start and end values.'))
                else:
                    steps = max(1, int(round(span / step_size)))
                    effective = span / steps
                    if abs(effective - step_size) > max(1e-6, step_size * 0.01):
                        notes.append(tf(
                            "Step size {step_size:g} {unit_label} is adjusted to an effective {effective:g} {unit_label} to fit the range.",
                            step_size=step_size,
                            effective=effective,
    ...
    if scan_type == "bond" and not start_auto and (float(start_val) <= 0 or float(end_val) <= 0):
        errors.append(t('Bond-scan distances must be greater than 0 Å.'))
    if steps < 1:
        errors.append(t('SCAN steps must be at least 1.'))
...
def render_scan_settings(prefix: str, current_scan_value: float | None = None, current_scan_error: str | None = None) -> dict[str, object]:
    ...
    with st.expander("SCAN settings", expanded=True):
        ...
        preset = st.selectbox(
            ...
            args=(prefix,),
            help=t('Template for intuitive input. Atom indices and ranges can still be adjusted after selection.'),
        )
    ...
    *,
    ...
    if preset == "Figure refresh only":
        render_workflow_preview_step(
            ...
            None,
            t('Load the existing trajectory and result CSV without running calculation stages.'),
            "existing result.csv",
        )
        render_workflow_preview_step(
            2,
            ...
            save_fig_on,
            t('Regenerate figures according to `SAVE_FIG_ON`.'),
            "fig_result.png",
        )
        render_workflow_preview_step(
            3,
            "Finalize run",
            None,
            t('Save the timing log and job metadata, then finish.'),
            "timing.log / molscout.log",
        )
        return

    input_detail = {
        "reactant_product": t('Create an initial path from reactant.xyz and product.xyz.'),
        "single_input": t('Load an existing trajectory or coordinate file.'),
        "single_input_with_result": t('Load an existing trajectory and result CSV.'),
    ...
    render_workflow_preview_step(
        ...
        None,
        t('Evaluate single-point energies for the input path or generated initial path.'),
        "result.csv",
    )
    render_workflow_preview_step(
        4,
        ...
        None,
        t('Extract candidate points for TS optimization and downstream analysis from result.csv.'),
    )
    render_workflow_preview_step(
        5,
        "TS optimization",
        do_ts,
        t('Controlled by `TSOPT_ON`. When ON, run TS optimization with Sella.'),
    ...
    render_workflow_preview_step(
        ...
        do_irc,
        t('Controlled by `IRC_ON`. When ON, run forward and reverse AdaptiveIRC.'),
    ...
    render_workflow_preview_step(
        ...
        pick_optpoints_on,
        t('Controlled by `PICK_OPTPOINTS_ON`. Extract candidate points and re-optimize them when needed.'),
    ...
    render_workflow_preview_step(
        ...
        do_vib,
        t('Controlled by `VIB_ON`. Calculate vibrational analysis and thermochemical corrections.'),
    ...
    render_workflow_preview_step(
        ...
        do_refine,
        t('Controlled by `REFINE_ENERGY_ON`. Run high-level single points with PySCF.'),
    ...
    render_workflow_preview_step(
        ...
        save_fig_on,
        t('Controlled by `SAVE_FIG_ON`. Save the profile figure and logs at the end.'),
...
def render_concat_workflow_preview_dialog(
    *,
    ...
    render_workflow_preview_step(
        ...
        "Input files",
        None,
        t('Load multiple .xyz / .traj files in the selected order.'),
    ...
    render_workflow_preview_step(
        ...
        "Concatenate frames",
        None,
        t('Concatenate the input files into one trajectory with `INIT_PATH_METHOD = CAT`.'),
    ...
    render_workflow_preview_step(
        ...
        "Evaluate single point energies",
        None,
        t('Evaluate single-point energies for all concatenated frames.'),
        "result.csv",
    )
    render_workflow_preview_step(
        4,
        ...
        do_opt,
        t('Controlled by `REFINE_INPUT_ON`. When ON, run structure optimization for each frame.'),
    )
    render_workflow_preview_step(
        5,
        "Pick and re-optimize optpoints",
    ...
    render_workflow_preview_step(
        ...
        "Vibration and thermo",
        do_vib,
        t('Controlled by `VIB_ON`. Calculate vibrational analysis and thermochemical corrections for batch targets.'),
        "*_vibsummary.txt / *_vib_*.xyz" if do_vib else "",
    )
    render_workflow_preview_step(
        7,
...
def render_module_settings(
    ...
    *,
    ...
    with st.expander("Detailed Module Settings", expanded=False):
        st.caption(t('Module settings equivalent to `core/default_config.py`, applied only to this job.'))
        ...
        values["fixed_atoms_text"] = st.text_input(
            "Fixed atoms",
            key=fixed_atoms_key,
            help=t('Comma-separated atom indices. Example: 0,1,2'),
...
def sidebar_monitor_fragment() -> None:
    ...
    with st.expander("GPU details", expanded=False):
        if gpu_rows:
            ...
            st.caption(t('No GPU metrics were detected in this environment.'))
...
def render_queue_panel() -> None:
    ...
    with cols[1]:
        if st.button(t(':material/refresh: Refresh view'), width="stretch"):
            st.rerun()
    ...
    if jobs:
        ...
        st.info(t('No jobs are currently in the queue.'))
...
def render_job_detail(
    ...
    *,
    ...
    if show_summary:
        ...
        if job.get("completion_reason") or job.get("status_message"):
            st.caption(
                tf(
                    "Result: {reason}",
                    reason=job.get("completion_reason") or "-",
                )
    ...
    with action_shell[1]:
        st.caption(t('Job actions'))
        action_cols = st.columns([1, 1, 1, 1, 2])
    if action_cols[0].button(t(':material/arrow_upward: Move up'), key=f"up_{job['job_id']}", disabled=index == 0 or job["status"] != "queued"):
        order = [item["job_id"] for item in jobs]
        order[index - 1], order[index] = order[index], order[index - 1]
        reorder_queue_for_session(session_id, order)
        st.rerun()
    if action_cols[1].button(t(':material/arrow_downward: Move down'), key=f"down_{job['job_id']}", disabled=index == len(jobs) - 1 or job["status"] != "queued"):
        order = [item["job_id"] for item in jobs]
        order[index + 1], order[index] = order[index], order[index + 1]
        reorder_queue_for_session(session_id, order)
        st.rerun()
    if action_cols[2].button(t(':material/stop_circle: Stop'), key=f"stop_{job['job_id']}", disabled=job["status"] != "running"):
        ...
        st.rerun()
    if action_cols[3].button(t(':material/delete: Delete'), key=f"delete_{job['job_id']}", disabled=job["status"] == "running"):
        delete_result = delete_job_from_queue(session_id, job["job_id"])
        if delete_result == "deleted":
            st.warning(t('Job deleted.'))
        elif delete_result == "deferred":
            st.info(t('Delete requested. The job will be removed automatically after a safe stop.'))
        else:
            st.info(t('This job no longer exists.'))
        st.rerun()
...
def render_session_overview(session: dict) -> None:
    ...
    st.download_button(
        t(':material/folder_zip: Download session ZIP'),
        zip_path.read_bytes(),
...
def render_pyscf_profile_editor(label: str, profile: dict, *, prefix: str) -> dict:
    ...
    level_shift_enabled = cols[2].checkbox(t('Use level shift'), value=level_shift_default is not None, key=f"{prefix}_scf_level_shift_enabled")
    profile["scf_level_shift"] = None
    if level_shift_enabled:
        profile["scf_level_shift"] = float(cols[3].number_input(t('Level-shift value'), min_value=0.0, value=float(number_default(level_shift_default, 0.1)), step=0.1, format="%.6g", key=f"{prefix}_scf_level_shift"))
    else:
        cols[3].caption(t('Level shift: none'))
...
@st.dialog(t('XC / basis candidates'))
...
def render_pyscf_copy_controls(session_id: str, config: dict, presets: dict) -> None:
    ...
    for index, (target_name, target_prefix) in enumerate(targets):
        ...
        selected_source = cols[index].selectbox(
            tf("Copy settings to {target_name} from", target_name=target_name),
            source_options,
        ...
        if cols[index].button(":material/content_copy: copy", key=f"{session_id}_{target_name}_copy_button", width="stretch"):
            ...
            st.success(tf(
                "Applied {source_label} to {target_name}. Save the config to persist the change.",
                source_label=labels[selected_source],
                target_name=target_name,
            ))
            st.rerun()
    cols[2].caption(t('Copy only updates the values shown on this screen. Use the save button to persist them.'))


def render_session_config(session: dict) -> None:
    st.markdown("## :material/settings: Session config")
    st.caption(t('PySCF settings are stored in this session metadata. Job-local JSON files are generated from these values.'))
    ...
    with st.container(border=True):
        if st.button(t(':material/help: XC / basis candidates'), key=f"{session['session_id']}_pyscf_candidates_help"):
        ...
        save_pressed = actions[0].button(t(':material/save: Save config'), type="primary", width="stretch")
        reset_pressed = actions[1].button(t(':material/restart_alt: Restore defaults'), width="stretch")
    ...
    if reset_pressed:
        ...
        clear_pyscf_profile_state(f"{session['session_id']}_pyscf")
        clear_pyscf_profile_state(f"{session['session_id']}_pyscf_high")
        st.success(t('PySCF settings for this session were restored to defaults.'))
        st.rerun()
    if reload_pressed:
        clear_pyscf_profile_state(f"{session['session_id']}_pyscf")
        clear_pyscf_profile_state(f"{session['session_id']}_pyscf_high")
...
def render_job_submission(session: dict) -> None:
    ...
    if path_reset_applied:
        st.success(t('Restored the session default settings.'))
    
    st.markdown(t('#### Workflow and input source settings'))
    ...
    if source_mode != t('Upload new files'):
        ...
        with st.container(border=True):
            if mode == "reactant_product":
                ...
                if labels:
                    existing_choice = st.selectbox(t('Use existing session file'), labels, key=f"{session_id}_existing")
                    existing_input = existing_inputs[labels.index(existing_choice)]
                else:
                    st.caption(t('This session does not have any existing `.traj` or `.xyz` files yet.'))

            else:
                st.caption(t('Figure refresh requires both a trajectory file and the CSV to redraw.'))
                cols = st.columns(2)
                ...
                if input_labels:
                    existing_input_choice = cols[0].selectbox(t('Existing trajectory / XYZ'), input_labels, key=f"{session_id}_fig_existing_input")
                    existing_input = existing_inputs[input_labels.index(existing_input_choice)]
                else:
                    cols[0].caption(t('This session does not have any existing `.traj` or `.xyz` files yet.'))
                if result_labels:
                    existing_result_choice = cols[1].selectbox(t('Existing result CSV'), result_labels, key=f"{session_id}_fig_existing_csv")
                    existing_result = existing_result_files[result_labels.index(existing_result_choice)]
                else:
                    cols[1].caption(t('This session does not have any existing `.csv` files yet.'))
    ...
    if source_mode == t('Upload new files'):
        ...
        with st.container(border=True):
            if mode == "reactant_product":
                ...
                input_file = st.file_uploader(t('Input `.traj` or `.xyz`'), type=["traj", "xyz"], key=f"{session_id}_input")
            else:
                st.caption(t('Figure refresh requires both a trajectory file and the CSV to redraw.'))
                cols = st.columns(2)
                input_file = cols[0].file_uploader(t('Input `.traj` or `.xyz`'), type=["traj", "xyz"], key=f"{session_id}_fig_input")
                result_file = cols[1].file_uploader(t('Existing result CSV'), type=["csv"], key=f"{session_id}_fig_csv")
    ...
    with st.container(border=True):
        ...
        if mode == "reactant_product":
            try:
                ...
                preview_scan_error = t('SCAN indices could not be read as an integer list.')
    ...
    with st.form(f"new_job_{session_id}", border=True):
        ...
        if preset == "Figure refresh only":
            ...
            st.info(t('Figure refresh only redraws plots from an existing trajectory and CSV. Calculation stages are not run.'), icon=":material/info:")
        else:
        ...
        pick_optpoints_on = extra_cols[1].checkbox(
            t('Extract optimized points (optpoints)'),
        ...
        with st.container(horizontal=True, horizontal_alignment="right"):
            reset_defaults_pressed = st.form_submit_button(t('Reset'), icon=":material/restart_alt:")
            save_defaults_pressed = st.form_submit_button(t('Set as session default'), icon=":material/save:")
            preview_pressed = st.form_submit_button(t('Preview workflow structure'), icon=":material/account_tree:")
            submitted = st.form_submit_button(t('Add job to queue'), type="primary", icon=":material/queue:")
    ...
    if save_defaults_pressed:
        ...
        st.success(t('Saved these settings as the session defaults.'))
        st.rerun()
    ...
    try:
        ...
        errors.append(t('Specify SCAN indices as a comma-separated list of integers.'))
        scan_indices = []
    if module_settings["init_path_method"] == "SCAN" and scan_indices:
        ...
        if len(scan_indices) != expected_count:
            errors.append(tf(
                "{scan_type} SCAN requires {expected_count} atom indices.",
                scan_type=module_settings["scan_type"],
    ...
    if module_settings["init_path_method"] == "SCAN":
        if current_scan_error and str(module_settings.get("scan_range_mode")) in {"relative_forward", "relative_window"}:
            errors.append(tf(
                "Could not read the current SCAN value: {error}",
    ...
    try:
        ...
        errors.append(t('Specify fixed atoms as a comma-separated list of integers.'))
        fixed_atoms = []
    if not any([do_path, do_ts, do_irc, do_vib, do_refine]) and preset != "Figure refresh only":
        errors.append(t('Select at least one workflow step.'))
    is_scan = module_settings["init_path_method"] == "SCAN"
    if mode == "reactant_product":
        if not do_path:
            errors.append(t('Enable Initial Path for reactant/product workflows.'))
        if source_mode == t('Upload new files'):
            if reactant_file is None:
                errors.append(t('Reactant file (Initial XYZ) is required.'))
            elif product_file is None and not is_scan:
                errors.append(t('Product file upload is required except in SCAN mode.'))
        else:
            ...
            if reactant_path is None or (product_path is None and not is_scan):
                errors.append(t('The selected sample case is incomplete.'))
    elif mode == "single_input":
        if source_mode == t('Upload new files') and input_file is None:
            errors.append(t('An input trajectory or XYZ file is required.'))
        if source_mode == t('Use existing session file') and existing_input is None:
            errors.append(t('Select an existing `.traj` or `.xyz` file from this session.'))
    else:
        if source_mode == t('Upload new files'):
            if input_file is None:
                errors.append(t('An input trajectory or XYZ file is required.'))
            if result_file is None:
                errors.append(t('Figure refresh requires an existing result CSV.'))
        else:
...
def render_concat_submission(session: dict) -> None:
    ...
    if concat_reset_applied:
        st.success(t('Restored the session default settings.'))
    st.caption(t('Concatenate multiple `.xyz` or `.traj` files into one trajectory and, if needed, run batch processing for all frames (optimization, vibrational analysis, energy refinement).'))

    st.markdown(t('#### Input source settings'))
    source_key = f"{session_id}_cat_source_mode"
    source_options = [t('Upload files')]
    if existing_inputs:
        source_options.append(t('Select existing session files'))
    ...
    with st.form(f"new_concat_job_{session_id}", border=True):
        ...
        if source_mode == t('Upload files'):
            uploaded_files = st.file_uploader(t('Upload `.traj` or `.xyz` files'), type=["traj", "xyz"], accept_multiple_files=True, key=f"{session_id}_cat_upload")
        else:
            labels = [f"{path.relative_to(session_dir(session_id))} ({file_size_label(path)})" for path in existing_inputs]
            if labels:
                selected_labels = st.multiselect(t('Select existing files (order is preserved)'), labels, key=f"{session_id}_cat_existing")
    ...
    if source_mode == t('Upload files'):
        if not uploaded_files:
            errors.append(t('Upload at least one file.'))
    else:
...
def render_session_jobs(session: dict) -> None:
    ...
    with header_cols[1]:
        if st.button(t(':material/refresh: Refresh jobs'), width="stretch"):
            refreshed_jobs = refresh_jobs(session["session_id"], jobs)

    if not jobs:
        st.info(t('This session has no jobs yet.'))
        return
    ...
    st.markdown(t('### Select jobs to inspect'))
    ...
    if not selected_indices:
        st.info(t('👆 Select one or more jobs in the table above to view details.'))
        return
    ...
    include_merged_csv = download_cols[1].checkbox(t('Include merged CSV'), key=f"{session['session_id']}_include_merged_csv", value=True)
    ...
    if download_cols[2].button(
        t(':material/folder_zip: Generate selected jobs ZIP'),
        key=f"{session['session_id']}_generate_selected_jobs_zip",
        width="stretch",
    ):
        with st.spinner(t('Building ZIP for selected jobs...')):
        ...
        st.success(t('ZIP for selected jobs has been created.'))

    if download_cols[3].button(
        t(':material/table: Generate merged CSV ZIP'),
        key=f"{session['session_id']}_generate_merged_csv_zip",
        width="stretch",
    ):
        with st.spinner(t('Building merged CSV ZIP...')):
        ...
        if merged_archive_path:
            ...
            st.success(t('Merged CSV ZIP has been created.'))
        else:
            st.session_state.pop(merged_state_key, None)
            st.info(t('No matching CSV file names were found.'))
    ...
    if selected_archive_path and selected_archive_path.exists():
        st.download_button(
            t(':material/download: Download selected jobs ZIP'),
    ...
    if merged_archive_path and merged_archive_path.exists():
        st.download_button(
            t(':material/download: Download merged CSV ZIP'),
...
def render_job_results(job: dict) -> None:
    ...
    if not files and not job_json_files:
        st.info(t('No outputs yet.'))
        return

    st.caption(f"result folder: `{run_dir}`")
    result_view = section_switch(
        "result view",
        [t('Overview'), "Logs", "Json", "XYZ", "Tables", "Images"],
        key=f"result_view_{job['job_id']}",
        captions={
            t('Overview'): t('Output folder overview and main log'),
            "Logs": ".log files",
            "Json": ".json files",
            "XYZ": ".xyz files",
            "Tables": t('CSV preview of reaction profiles and derived data'),
            "Images": t('Saved figures and plots'),
        },
    )

    if result_view == t('Overview'):
